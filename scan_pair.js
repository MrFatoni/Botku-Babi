...
(async () => {
  ...
  for (let i = 0; i < nb_pair; i += step) {
    ...
    const [balance,balance2,symbol,symbol2,addresse,addresse2] = ethers.utils.defaultAbiCoder.decode(
      ["uint256[]","uint256[]","string[]","string[]","address[]","address[]"],
      returnedData
    );
    ...
    for (let j = 0; j < balance.length; j++) {
     ...
     if (balance[j].gt(100) && balance2[j].gt(100)) //100 wei
pairx = `${ethers.utils.formatEther(balance[j])} ${symbol[j]} = https://etherscan.io/address/${addresse[j]}
${ethers.utils.formatEther(balance2[j])} ${symbol2[j]} = https://etherscan.io/address/${addresse2[j]}

`;
console.log(pairx);

fs.appendFileSync("pairx.txt",'\n' + [pairx].join("\n"));



  }
} catch (errd) {
  console.log(errd.code);
  continue;
}
 console.log(i);
  }
